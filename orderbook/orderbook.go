...
func (ob *OrderBook) ProcessMarketOrder(accountId string, side Side, quantity decimal.Decimal) (done []*Order, partial *Order, partialQuantityProcessed, quantityLeft decimal.Decimal, err error) {
	if quantity.Sign() <= 0 {
	...
	for quantity.Sign() > 0 && sideToProcess.Len() > 0 {
		bestPrice := iter()
		ordersDone, partialDone, partialProcessed, quantityLeft := ob.processQueue(accountId, bestPrice, quantity)
...
//	partialQuantityProcessed - if partial order is not nil this result contains processed quatity from partial order
func (ob *OrderBook) ProcessLimitOrder(accountId string, side Side, quantity, price decimal.Decimal) (done []*Order, partial *Order, partialQuantityProcessed decimal.Decimal, err error) {
	...
	for quantityToTrade.Sign() > 0 && sideToProcess.Len() > 0 && comparator(bestPrice.Price()) {
		ordersDone, partialDone, partialQty, quantityLeft := ob.processQueue(accountId, bestPrice, quantityToTrade)
		done = append(done, ordersDone...)
		partial = partialDone
	...
	if quantityToTrade.Sign() > 0 {
		o := NewOrder(orderID, accountId, side, quantityToTrade, price, time.Now().UTC())
		if len(done) > 0 {
		...
		done = append(done, NewOrder(orderID, accountId, side, quantity, totalPrice.Div(totalQuantity), time.Now().UTC()))
	}
	return
}

func (ob *OrderBook) ProcessLimitOrderNew(accountId string, side Side, quantity, price decimal.Decimal) (done []*Order, partial *Order, o *Order, partialQuantityProcessed decimal.Decimal, err error) {
	...
	orderToReturn := NewOrder(orderID, accountId, side, quantityToTrade, price, time.Now().UTC())
	...
	return done, partial, orderToReturn, partialQuantityProcessed, nil
}

func (ob *OrderBook) processQueue(accountId string, orderQueue *OrderQueue, quantityToTrade decimal.Decimal) (done []*Order, partial *Order, partialQuantityProcessed, quantityLeft decimal.Decimal) {
	...
	for orderQueue.Len() > 0 && quantityLeft.Sign() > 0 {
		...
		if quantityLeft.LessThan(headOrder.Quantity()) {
			partial = NewOrder(headOrder.ID(), headOrder.AccountId(), headOrder.Side(), headOrder.Quantity().Sub(quantityLeft), headOrder.Price(), headOrder.Time())
